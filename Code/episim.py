...
"""
state[k]=0 if the k-th node is suseptible
state[k]=1 if the k-th node is infected
state[k]=2 if the k-th node is recovered
"""
...
# the simulation will run as long as the number of infected > 0
    while len(infected) > 0:
        ...
        for index, value in enumerate(state):
            nodes[index] = value
        values = [nodes.get(node, 0) for node in G.nodes()]

        plt.figure(figsize=(20, 20))
        pos = nx.spring_layout(G)
        nx.draw_networkx_nodes(G, pos, node_color=values, node_size=100)

        nx.draw_networkx_edges(G, pos)
        nx.draw_networkx_labels(G, pos, font_size=8, font_family='sans-serif')
        plt.title('Epidemiology Network')
        yellow_patch = mpatches.Patch(color='yellow', label='recovered')
        violet_patch = mpatches.Patch(color=(68 / 255, 1 / 255, 84 / 255), label='Susceptible')
        blue_patch = mpatches.Patch(color=(42 / 255, 120 / 255, 142 / 255), label='Infected')

        plt.legend(handles=[yellow_patch, violet_patch, blue_patch])

        # plt.savefig('network_epi.png')
        plt.show()
